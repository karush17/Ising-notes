...
\begin{proof}
    ...
    \begin{gather}
        \nabla_{\theta}\underset{s_{t},a_{t}}{\textbf{E}}[\sum_{i,j} \sigma^{\pi_{i}}\sigma^{\pi_{j}}] \leq 2L\underset{s_{t},a_{t}}{\textbf{E}}[\sum_{i,j} \pi^{\theta}_{*}(a_{t}|s_{t})\nabla_{\theta}\log\pi^{\theta}_{*}(a_{t}|s_{t})] \label{eq:bound}               
    \end{gather}
    Thus,
    \begin{gather}
        \nabla_{\theta}J(\theta) \leq \underset{s_{t},a_{t}}{\textbf{E}}[\nabla_{\theta}(\log \pi^{\theta}_{i}(a_{t}|s_{t})(Q^{\pi}(s_{t},a_{t}) + b^{\theta}(s_{t})) + 2L\underset{s_{t},a_{t}}{\textbf{E}}[\pi^{\theta}_{*}(a_{t}|s_{t})\log \pi^{\theta}_{*}(a_{t}|s_{t})] \nonumber
...
Note that \autoref{four} makes a strong Lipschitz assumption which may not hold true in practical scenarios, Moreover, the bound depends on optimal policy $\pi^{\theta}_{*}(a_{t}|s_{t})$ which is not observed during the agent's interactions with the environment. Hence, the bound obtained in \autoref{four} is not a tight one and intractable in nature.
