...
\begin{proof}
    ...
    \begin{gather}
    I(\pi^{\theta}_{1}(a_{t}|s_{t});\pi^{\theta}_{N}(a_{t}|s_{t})) = - \sum_{a_{t}} \pi^{\theta}_{1}(a_{t}|s_{t}) \log(\pi^{\theta}_{N}(a_{t}|s_{t})\pi^{\theta}_{N-1}(a_{t}|s_{t})...\pi^{\theta}_{1}(a_{t}|s_{t})) \nonumber \\
    I(\pi^{\theta}_{1}(a_{t}|s_{t});\pi^{\theta}_{N}(a_{t}|s_{t})) = - \sum_{a_{t}} \pi^{\theta}_{1}(a_{t}|s_{t}) \log\prod_{n=1}^{N}(\pi^{\theta}_{n}(a_{t}|s_{t})) \nonumber \\
    I(\pi^{\theta}_{1}(a_{t}|s_{t});\pi^{\theta}_{N}(a_{t}|s_{t})) = - \sum_{a_{t}} \pi^{\theta}_{1}(a_{t}|s_{t}) \sum_{n=1}^{N}\log(\pi^{\theta}_{n}(a_{t}|s_{t})) \nonumber \\
    I(\pi^{\theta}_{1}(a_{t}|s_{t});\pi^{\theta}_{N}(a_{t}|s_{t})) = - \underset{\pi^{\theta}_{1}}{\textbf{E}}[\sum_{n=1}^{N}\log(\pi^{\theta}_{n}(a_{t}|s_{t}))] \label{eq:info}
    \end{gather}
    We now add a new level $h_{N+1}$ in the hierarchy $H$ and repeat the steps presented above to obtain \autoref{eq:newinfo},
    \begin{gather}
        I(\pi^{\theta}_{1}(a_{t}|s_{t});\pi^{\theta}_{N}(a_{t}|s_{t})) = - \underset{\pi^{\theta}_{1}}{\textbf{E}}[\sum_{n=1}^{N+1}\log(\pi^{\theta}_{n}(a_{t}|s_{t}))] \label{eq:newinfo}        
    \end{gather} 
    On comparing \autoref{eq:info} and \autoref{eq:newinfo} we observe that the addition of level $h_{N+1}$ increases the magnitude of estimate in expectation of the primary level policy $\pi^{\theta}_{1}(a_{t}|s_{t})$ which completes the proof.
\end{proof}
